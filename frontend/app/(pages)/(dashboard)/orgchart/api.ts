...
export const createOrgChartDraft = async (data: CreateDraftRequest): Promise<OrgChartDraft> => {
  const response = await fetchWithProxy("/orgchart/drafts", {
    method: "POST",
...
export const removeDraftChange = async (draftId: number, userId: number): Promise<void> => {
