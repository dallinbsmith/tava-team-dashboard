...
} from "@/app/(pages)/(dashboard)/admin/invitations/types";
import {
  JiraSettings,
...
import {
  ...
  ResponseStatus,
} from "@/app/(pages)/(dashboard)/calendar/types";
import {
  TimeOffRequest,
  TimeOffStatus,
...
export const createInvitation = (data: CreateInvitationRequest) =>
  mutate<Invitation>("/invitations", "POST", data);
...
export const getUserJiraTasks = (userId: number, max = 50) =>
  get<JiraIssue[]>(`/jira/tasks/user/${userId}?max=${max}`);
export const getTeamJiraTasks = (maxPerUser = 20) =>
  get<TeamTask[]>(`/jira/tasks/team?max_per_user=${maxPerUser}`);
export const getJiraProjects = () => get<JiraProject[]>("/jira/projects");
export const getProjectJiraTasks = (projectKey: string, max = 50) =>
  get<JiraIssue[]>(`/jira/projects/${projectKey}/tasks?max=${max}`);
export const getJiraEpics = (max = 100) => get<JiraIssue[]>(`/jira/epics?max=${max}`);
export const getJiraOAuthAuthorizeURL = () =>
  get<JiraOAuthAuthorizeResponse>("/jira/oauth/authorize");
...
export const updateUserJiraMapping = async (
  ...
  const res = await fetchWithProxy(`/jira/users/${userId}/mapping`, {
    method: "PUT",
...
export const createOrgChartDraft = (data: CreateDraftRequest) =>
  mutate<OrgChartDraft>("/orgchart/drafts", "POST", data);
export const updateOrgChartDraft = (id: number, data: UpdateDraftRequest) =>
  mutate<OrgChartDraft>(`/orgchart/drafts/${id}`, "PUT", data);
export const deleteOrgChartDraft = (id: number) => del(`/orgchart/drafts/${id}`);
export const addDraftChange = (draftId: number, change: AddDraftChangeRequest) =>
  mutate<DraftChange>(`/orgchart/drafts/${draftId}/changes`, "POST", change);
export const removeDraftChange = (draftId: number, userId: number) =>
  del(`/orgchart/drafts/${draftId}/changes/${userId}`);
...
// Calendar
const calendarQuery = (start: Date, end: Date) =>
...
export const getCalendarEvents = async (start: Date, end: Date) =>
  (await get<CalendarEventsResponse>(calendarQuery(start, end))).events || [];
export const getCalendarEventsWithMetadata = (start: Date, end: Date) =>
  get<CalendarEventsResponse>(calendarQuery(start, end));

// Tasks
export const createTask = (data: CreateTaskRequest) =>
  mutate<Task>("/calendar/tasks", "POST", data);
export const getTask = (id: number) => get<Task>(`/calendar/tasks/${id}`);
export const updateTask = (id: number, data: UpdateTaskRequest) =>
  mutate<Task>(`/calendar/tasks/${id}`, "PUT", data);
export const deleteTask = (id: number) => del(`/calendar/tasks/${id}`);

// Meetings
export const createMeeting = (data: CreateMeetingRequest) =>
  mutate<Meeting>("/calendar/meetings", "POST", data);
export const getMeeting = (id: number) => get<Meeting>(`/calendar/meetings/${id}`);
export const updateMeeting = (id: number, data: UpdateMeetingRequest) =>
  mutate<Meeting>(`/calendar/meetings/${id}`, "PUT", data);
export const deleteMeeting = (id: number) => del(`/calendar/meetings/${id}`);
export const respondToMeeting = (meetingId: number, response: ResponseStatus) =>
  postVoid(`/calendar/meetings/${meetingId}/respond`, { response });

// Time Off
export const createTimeOffRequest = (data: CreateTimeOffRequest) =>
  mutate<TimeOffRequest>("/time-off", "POST", data);
export const getMyTimeOffRequests = (status?: TimeOffStatus) =>
  get<TimeOffRequest[]>(`/time-off${status ? `?status=${status}` : ""}`);
...
export const reviewTimeOffRequest = (id: number, review: ReviewTimeOffRequest) =>
  mutate<TimeOffRequest>(`/time-off/${id}/review`, "PUT", review);
